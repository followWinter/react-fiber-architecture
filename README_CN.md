...
其他关键特性比如暂停，放弃，或者重用的能力作为新的更新到来；给不同蕾西更新赋予不同优先级的能力；和新的并发原语。
...
请注意我不再 React 团队，并没有从作者询问任何东西。**这不是一个官方文档**。我有询问过 React 团队的成员去评论它的正确。

这个文档还在进程中。**Fiber 是一个正在进行的项目，将会可能在完成之前执行重大重构。** 我在这里记录它的设计也在进行中。提升和建议都是非常欢迎的。

我的目标是在读完整个文档之后，你将会足够理解 Fiber 去[跟随它的实现]()，最终甚至可以给 React 贡献。

### 先决条件

我强烈建议你在继续之前先熟悉瞎买呢的资源：
...
React 的 API 的核心思想是认为 update 导致整个 app 重新渲染。这允许开发者进行有效声明式，而不是担心怎样去有效的转移 app 从任意特定状态到其他（A 到 B，B 到 C，C 到 A，等等）。



实际上，在每一次该改变的时候重新渲染整个 app 只在最细小的 app 有效；在一个真实世界的 app，对于性能来说是禁止的。React 做了优化，重新渲染创建整个 app 的外观并维护好的性能。这些大部分的优化都是**调和**过程的一部分。


调试算法是大众理解的"虚拟 DOM"的背后。一个高级别的描述就像这样：当你渲染一个 React 应用，描述整个 app 的节点被创建保存在内存。然后树被刷新到渲染环境 -- 比如，在浏览器应用场景，它转化为一个集合的 DOM 操作。当 app 被更新（通常通过`setState`），一个新的树被生成。新的树和前一个树对比，计算出哪些操作需要更新到已经渲染的 app。

尽管 Fiber 从头来时重写协调，[描述在 React 文档](https://facebook.github.io/react/docs/reconciliation.html)高层级的算法大部分都一样。关键是：
...
React 可以支持这么多的目标是因为它设计为调和和渲染是分开的步骤。协调器负责计算树的那一部分发生该拜年的工作；渲染器使用这些信息去真实更新渲染的 app。
...
React 的[设计理念](https://facebook.github.io/react/contributing/design-principles.html#scheduling)在这个课题很好，我将只在这里引用它：

> 在当前的实现中，React 递归遍历整棵树树，并在单个 tick 中调用整个更新的树的 render 函数。然而在未来，它可能开始延迟一些更新去避免掉帧。
>
> 这是 React 设计的常见主题。一些流行的库实现"推"方法，在新数据可用时执行计算。React，然而，坚持"pull"方法，这样计算可以被推迟直到必要。
> 
> React 不是通用数据处理库。它是一个用于构建用户接口的库。我们认为它独特的点是知道在 app 中，哪些计算是现在相关的，哪些不是。
> 
> 如果一些东西在屏幕之外，我们可以延迟和它相关的逻辑。如果数据来的比帧率还快，我们可以合并和批量更新。我们可以比不重要的背景工作（比如渲染只是从网络加载来的新内容）优先处理来自用户交互的工作（比如点击按钮触发的动画）。

关键点是：

- 在一个 UI，每一个更新被立即应用是不需要的；实际上，这么做很浪费，导致掉帧并歼敌用户体验。
- 不同类型的更新有不同的优先级 - 一个动画更新需要比数据存储的更新更快的完成。
- 一个基于推的方式需要 app（你，程序员）去决定怎样调度工作。一个基于拉的方式允许框架（React）去灵活为你做这些决定。

React 当前没有充分利用调度的优势；一个更新导致整个子树立即被重新渲染。为了重逢利用调用而大修 React 的核心算法是 Fiber 背后的驱动思想。 
---

现在我们准备深入 FIber 的实现。下一个章节比我们已经讨论的更加技术型。请确保你对前面的材料感觉舒适。
